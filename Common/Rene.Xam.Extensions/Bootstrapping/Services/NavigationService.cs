...
namespace Rene.Xam.Extensions.Bootstrapping.Services
{
    public class NavigationService : INavigationService
    {
        ...
        public async Task PushWithBackButtonAsync<TViewModel>() where TViewModel : class, IViewModelBase
        {

            var view = _viewFactory.Resolve<TViewModel>();
            await Navigation.PushAsync(view);
        }
        public async Task PushWithBackButtonAsync<TViewModel, TKArguments>(TKArguments args) where TViewModel : class, IArgumentViewModel<TKArguments>
        {

            var view = _viewFactory.Resolve<TViewModel, TKArguments>(args);
            await Navigation.PushAsync(view);
        }
        public async Task PushWithBackButtonAsync(Type viewModelType)
        {
            var view = _viewFactory.Resolve(viewModelType);
            await Navigation.PushAsync(view);
        }
        public async Task PushWithBackButtonAsync<TKArguments>(Type viewModelType, TKArguments args)
        {
            var view = _viewFactory.Resolve<TKArguments>(viewModelType, args);
            await Navigation.PushAsync(view);
        }
